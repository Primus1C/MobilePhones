...
Процедура ВывестиРодительскиеДокументы(ДокументСсылка)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ Ссылка, Проведен, ПометкаУдаления, Представление, #Валюта, #Сумма, #Количество, #Поставка,""" + МетаданныеДокумента.Имя + """ КАК Метаданные
	...
	ИначеЕсли МетаданныеДокумента.Реквизиты.Найти("Валюта") <> Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Валюта", "Валюта КАК ВалютаДокумента"); 
	// © Primus1C  ОКОНЧАНИЕ ◄	
	Иначе
	...
	ИначеЕсли МетаданныеДокумента.Реквизиты.Найти("Сумма") <> Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Сумма", "Сумма КАК СуммаДокумента");
	// © Primus1C  ОКОНЧАНИЕ ◄	
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Сумма", "0 КАК СуммаДокумента");
	КонецЕсли;
	
	// © Primus1C  26.07.2023  НАЧАЛО ►
	Если МетаданныеДокумента.Реквизиты.Найти("Количество") <> Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Количество", "Количество КАК КоличествоДокумента"); 
	ИначеЕсли МетаданныеДокумента.Реквизиты.Найти("ВКД_Количество") <> Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Количество", "ВКД_Количество КАК КоличествоДокумента"); 
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Количество", "0 КАК КоличествоДокумента");
	КонецЕсли;     
	Если МетаданныеДокумента.Реквизиты.Найти("Поставка") <> Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Поставка", "Поставка КАК Поставка");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Поставка", "0 КАК Поставка");
	КонецЕсли;     
	// © Primus1C  ОКОНЧАНИЕ ◄
	...
	Если Выборка.Следующий() Тогда		
		...
		// © Primus1C  26.07.2023  НАЧАЛО ►
		СтрокаДерева.КоличествоДокумента = Выборка.КоличествоДокумента;  
		//Если ТипЗнч(Выборка.Ссылка) = Тип("ДокументСсылка._Инвойс") Тогда
		//	Если Выборка.Ссылка.ВидОперации <> Перечисления.ВидыОперацииИнвойс.Товар Тогда
		//		СтрокаДерева.КоличествоДокумента = СтрШаблон("<%1>", Нрег(Выборка.Ссылка.ВидОперации));
		//	КонецЕсли;	
		//КонецЕсли;
		СтрокаДерева.Поставка = Выборка.Поставка;
		//УстановитьЗначенияСУчетомПоставки(СтрокаДерева.КоличествоДокумента, СтрокаДерева.СуммаДокумента		
		// © Primus1C  ОКОНЧАНИЕ ◄
...
Процедура ВывестиПодчиненныеДокументы(СтрокаДерева)
	...
	Для Каждого КлючИЗначение ИЗ КэшПоТипамДокументов Цикл
		Запрос.Текст = Запрос.Текст + ?(Запрос.Текст = "", "
					...
					|ОБЪЕДИНИТЬ ВСЕ
					|ВЫБРАТЬ") + "
					|Дата, Ссылка, Проведен, ПометкаУдаления, Представление,  """ + КлючИЗначение.Ключ + """ КАК Метаданные, #Валюта, #Сумма, #Количество, #Поставка 
		...
		ИначеЕсли мКэшРеквизитовДокумента[КлючИЗначение.Ключ]["ВалютаДенежныхСредств"] Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Валюта", "ВалютаДенежныхСредств КАК ВалютаДокумента");
		//// © Primus1C  06.07.2023  НАЧАЛО ►
		ИначеЕсли мКэшРеквизитовДокумента[КлючИЗначение.Ключ]["Валюта"] Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Валюта", "Валюта КАК ВалютаДокумента"); 
		//// © Primus1C  ОКОНЧАНИЕ ◄	
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Валюта", "NULL КАК ВалютаДокумента");
		...
		Если мКэшРеквизитовДокумента[КлючИЗначение.Ключ]["СуммаДокумента"] Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Сумма", "СуммаДокумента КАК СуммаДокумента");
		// © Primus1C  06.07.2023  НАЧАЛО ►
		ИначеЕсли мКэшРеквизитовДокумента[КлючИЗначение.Ключ]["Сумма"] Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Сумма", "Сумма КАК СуммаДокумента");
		// © Primus1C  ОКОНЧАНИЕ ◄	
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Сумма", "0 КАК СуммаДокумента");
		...
		// © Primus1C  26.07.2023  НАЧАЛО ►
		Если мКэшРеквизитовДокумента[КлючИЗначение.Ключ]["Количество"] Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Количество", "Количество КАК КоличествоДокумента");
		ИначеЕсли мКэшРеквизитовДокумента[КлючИЗначение.Ключ]["ВКД_Количество"] Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Количество", "ВКД_Количество КАК КоличествоДокумента");
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Количество", "0 КАК КоличествоДокумента");
		КонецЕсли;
		Если мКэшРеквизитовДокумента[КлючИЗначение.Ключ]["Поставка"] Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Поставка", "Поставка КАК Поставка");
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Поставка", "0 КАК Поставка");
		КонецЕсли;     
	...
	Пока Выборка.Следующий() Цикл
		Если мУжеВСписке[Выборка.Ссылка] = Неопределено Тогда
			...
			// © Primus1C  26.07.2023  НАЧАЛО ►
			НоваяСтрока.КоличествоДокумента = Формат(Выборка.КоличествоДокумента, "");  
			//Если ТипЗнч(Выборка.Ссылка) = Тип("ДокументСсылка._Инвойс") Тогда
			//	Если Выборка.Ссылка.ВидОперации <> Перечисления.ВидыОперацииИнвойс.Товар Тогда
			//		СтрокаДерева.КоличествоДокумента = СтрШаблон("<%1>", Нрег(Выборка.Ссылка.ВидОперации));
			//	КонецЕсли;	
			//КонецЕсли;
			НоваяСтрока.Поставка = Выборка.Поставка; 
			// © Primus1C  ОКОНЧАНИЕ ◄
...
Процедура ОткрытьФормуДокумента()
	...
	Исключение
		...
		ОбщегоНазначенияСервер.СообщитьОбОшибке(ДокументСсылка, ОписаниеОшибки());
...
Процедура ДополнитьКэшМетаданных(МетаданныеДокумента, ИмяДокумента)
	...
	Если РеквизитыДокумента = Неопределено Тогда
		...
		РеквизитыДокумента.Вставить("СуммаЗакупки", 	МетаданныеДокумента.Реквизиты.Найти("СуммаЗакупки") <> Неопределено);  
		РеквизитыДокумента.Вставить("Количество", 		МетаданныеДокумента.Реквизиты.Найти("Количество") <> Неопределено);  
		РеквизитыДокумента.Вставить("ВКД_Количество", 	МетаданныеДокумента.Реквизиты.Найти("ВКД_Количество") <> Неопределено);  
		РеквизитыДокумента.Вставить("Поставка", 		МетаданныеДокумента.Реквизиты.Найти("Поставка") <> Неопределено);  
		// © Primus1C  ОКОНЧАНИЕ ◄
...
Функция ОтменаПроведенияСервер(ДокументСсылка)
	...
	Исключение
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru = 'Невозможно заблокировать документ.'; en = 'Unable to lock the document.'");
		Сообщение.Сообщить();
	КонецПопытки; 
...
Процедура ВывестиДляТекущего(Команда)	
	...
	Если ОсновнойДокументЕщеДоступен() Тогда
		...
		ВывестиДеревоДокументов();
	Иначе
		ЗакрытьФормуСПредупреждением(
		НСтр("ru = 'Документ, для которого сформирован отчет о структуре подчиненности был удален, или же стал недоступен.'; en = 'The document for which the report on subordination structure was generated has been deleted or has become unavailable.'"));
		//НСтр("ru = 'Документ, для которого сформирован отчет о структуре
		//							 |подчиненности был удален, или же стал недоступен.'"));
	КонецЕсли;
...
Процедура Провести(Кнопка)
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru = 'Не удалось провести документ %Документ%!'; en = 'Failed to post document %Документ%!'");;
...
Процедура ОтменаПроведения(Кнопка)
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю();  
		Сообщение.Текст = НСтр("ru = 'Не удалось сделать непроведенным документ %Документ%!'; en = 'Failed to make document %Документ% unposted!'");;
		Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%Документ%", Элементы.ДеревоДокументов.ТекущиеДанные.ДокументПредставление);
		Сообщение.Сообщить();
...
Процедура УстановитьПометкуУдаления(Кнопка)	
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = НСтр("ru = 'Не удалось установить пометку удаления на документ %Документ%!'; en = 'Failed to set deletion mark on document %Документ%!'");;
		Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%Документ%", Элементы.ДеревоДокументов.ТекущиеДанные.ДокументПредставление);
		Сообщение.Сообщить();
...
Процедура УстановитьЗначенияСУчетомПоставки(РеквКоличество, РеквСумма, ДокументСсылка, НомерПоставки)

	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.Отправка") ИЛИ ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтправкаAWB") Тогда
		Запр = Новый Запрос; 
		Запр.УстановитьПараметр("Отправка", ДокументСсылка);
		Запр.УстановитьПараметр("Поставка", НомерПоставки);
		Запр.Текст = "ВЫБРАТЬ
		             |	ОтправкаИнвойсы.Поставка,
		             |	СУММА(ОтправкаИнвойсы.Количество) КАК Количество,
		             |	СУММА(ОтправкаИнвойсы.Сумма) КАК Сумма
		             |ИЗ
		             |	Документ.Отправка.Инвойсы КАК ОтправкаИнвойсы
		             |ГДЕ
		             |	ОтправкаИнвойсы.Ссылка = &Отправка
		...
		Если Выборка.Следующий() Тогда
			РеквКоличество 	= Выборка.Количество; 
			РеквСумма 		= Выборка.Сумма; 
		КонецЕсли;
	Иначе
		РеквКоличество 	= Выборка.КоличествоДокумента; 
		РеквПоставка 	= Выборка.Поставка; 
	КонецЕсли;	
